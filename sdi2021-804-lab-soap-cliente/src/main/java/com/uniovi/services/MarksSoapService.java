...
public class MarksSoapService extends WebServiceGatewaySupport {
	 ...
	 public GetMarksResponse getMarks(String dni) {
		 ...
		 request.setDni(dni);
		 GetMarksResponse response = (GetMarksResponse)getWebServiceTemplate().marshalSendAndReceive(serviceEndpoint, request, new SoapActionCallback(serviceSoapAction));
		 return response;
	 }
}
